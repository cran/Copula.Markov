Clayton.Markov.GOF.binom = function(Y,k=3,size,B=200,GOF.plot=FALSE){
  
  res = Clayton.Markov.MLE.binom(Y = Y, size = size, k=3, method="nlm", plot=FALSE, GOF=TRUE)
  p.hat = res$p[1]
  ...
  for( b in c(1:B)){
    ...
    res.boot = try(Clayton.Markov.MLE.binom(Y = Y.boot, size = size, k=3,method="nlm", plot=FALSE, GOF=GOF.plot))
